 ...
 * NOT persisted to Airtable.
 * Session-level studio tray state. No commerce language.
 *
 * v2.4 addition:
 *   Holds ONE active JewelryBuildDraft (currentDraft) so a draft survives
 *   the tray → builder navigation within the same provider. Tray item logic
 *   is unchanged. Only ONE draft at a time — no draft list, no persistence.
 */
...
export function WorkTrayProvider({ children }) {
  ...
  // v2.4: single active jewelry build draft (app-state only, not persisted)
  ...
  // v2.4: draft setters — pure replacement, single draft only
