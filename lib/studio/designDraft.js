...
export const STYLE_PREFERENCE = Object.freeze({
  ...
  MINIMAL: 'minimal',
  STATEMENT: 'statement',
  // --- Clean 5E — Design Intent Layer (additive values only; approved) ---
  // "Bold / נוכח" deliberately maps to the EXISTING 'statement' value.
  LUXURY: 'luxury',
  DELICATE: 'delicate',
  HALO: 'halo',
  SOLITAIRE: 'solitaire',
  THREE_STONE: 'threeStone',
  TENNIS: 'tennis',
  CUSTOM: 'custom',
});
...
export const STONE_USAGE = Object.freeze({
  ...
  OPTIONAL: 'optional',
  NONE: 'none',
  // --- Clean 5E — additive value only (approved): "שהמערכת תציע". Behaves
  // like a stone-permitting usage everywhere (only NONE forces metal-only).
  SYSTEM_SUGGEST: 'systemSuggest',
});
...
// ===========================================================================
// Clean 5E — Design Freedom Level (additive; approved minimal change)
// ===========================================================================
// How strict or creative the FUTURE render/design engine should be. Stored on
// the brief and threaded into concept/output/render-brief TEXT only — no
// render engine, no network, no new store. Canonical English values; Hebrew
// labels live in labels.js (INTENT_HE.freedom).
export const FREEDOM_LEVEL = Object.freeze({
  LOCKED: 'locked', // מדויק — follow stone data/reference closely
  GUIDED: 'guided', // מאוזן — keep core idea, tasteful improvements (default)
  CREATIVE: 'creative', // פתוח — stronger interpretation, alt settings
  EXPLORATORY: 'exploratory', // חופשי — bold collection-style, production-aware
});

export const FREEDOM_LEVEL_VALUES = Object.freeze(Object.values(FREEDOM_LEVEL));

export const DEFAULT_FREEDOM_LEVEL = FREEDOM_LEVEL.GUIDED;

export function isValidFreedomLevel(v) {
  return FREEDOM_LEVEL_VALUES.includes(v);
}
...
export function emptyBrief() {
  return {
    ...
    // --- Clean 5E — Design Intent Layer (additive; approved) ---
    // Render/design freedom level. Always a valid value; defaults to GUIDED.
    freedomLevel: DEFAULT_FREEDOM_LEVEL, // one of FREEDOM_LEVEL_VALUES
...
export function normalizeOutput(raw) {
  ...
  return {
    ...
    // Additive + defensive: older saved outputs without these still normalize
    // (structured → null, text → ''), so nothing breaks on load.
    renderBriefStructured:
      raw.renderBriefStructured && typeof raw.renderBriefStructured === 'object'
        ? {
            jewelryType: str(raw.renderBriefStructured.jewelryType),
            stonePlacement: str(raw.renderBriefStructured.stonePlacement),
            metal: str(raw.renderBriefStructured.metal),
            settingStyle: str(raw.renderBriefStructured.settingStyle),
            proportions: str(raw.renderBriefStructured.proportions),
            cameraAngle: str(raw.renderBriefStructured.cameraAngle),
            lighting: str(raw.renderBriefStructured.lighting),
            background: str(raw.renderBriefStructured.background),
            styleDescriptor: str(raw.renderBriefStructured.styleDescriptor),
          }
        : null,
    renderBriefText: str(raw.renderBriefText),
    productionNotes: str(raw.productionNotes),
...
export function normalizeBrief(raw) {
  ...
  return {
    ...
    // --- Clean 5E — Design Intent Layer (additive; approved) ---
    freedomLevel: isValidFreedomLevel(raw.freedomLevel)
      ? raw.freedomLevel
...
export function computeInputSignature(brief, trayItems) {
  ...
  // Clean 5E (approved) — freedom level participates in staleness ONLY when
  // it differs from the default. This keeps every legacy stored signature
  // (computed before this token existed) valid — no false stale banner on
  // deploy — while any move to/from a non-default level still changes the
  // signature and correctly marks concepts/outputs stale.
  if (b.freedomLevel && b.freedomLevel !== DEFAULT_FREEDOM_LEVEL) {
    parts.push(`freedom=${b.freedomLevel}`);
  }
  return parts.join('||');
}
