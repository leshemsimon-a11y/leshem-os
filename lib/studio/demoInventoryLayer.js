...
export const demoInventorySeed = Object.freeze([
  ...
  },
  {
    ...
    titleHe: 'טורמלין פאראיבה אובלי',
    stoneType: 'Paraiba Tourmaline',
    stoneTypeHe: 'טורמלין פאראיבה',
    species: 'Tourmaline',
...
export function getDemoActivityFeed() {
  ...
  return [
    ...
    {
      id: 'activity-ruby',
      textHe: 'רובי קושן נשלח למגש העבודה',
      tone: 'work-tray',
    },
    {
      id: 'activity-diamond',
      textHe: 'יהלום אמרלד קאט נפתח לבדיקה',
      tone: 'inspect',
    },
    {
      id: 'activity-sapphire',
      textHe: 'ספיר כחול סומן כזמין מספק',
      tone: 'supplier',
    },
  ];
}
...
export function getSourceLabelHe(sourceType) {
  if (sourceType === 'supplier') return 'ספק';
  if (sourceType === 'client-owned') return 'של לקוח';
  return 'מלאי שלנו';
}

// Terminology & Microcopy QA Pass — a SEPARATE, optional small badge for the
// "virtual supplier stock" context, so the main source label can stay short
// ("ספק") without every simple source badge being overloaded with it. Callers
// that have room for a second small badge (card pill rows, inspector badge
// rows) may render this alongside getSourceLabelHe(); it returns null for
// every other source type, so callers can safely `.filter(Boolean)` it in.
export function getSourceContextBadge(sourceType) {
  if (sourceType === 'supplier') return 'וירטואלי';
  return null;
}
