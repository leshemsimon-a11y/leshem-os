...
export function buildOutputPack(project) {
  ...
  // Clean 8C — cluster design language when the composition has multiple
  // stones (or an explicit cluster style value, checked defensively).
  if (stonesEn.length > 1 || brief.styleDirection === 'cluster') {
    promptLines.push(
      'Compose the stones as a cohesive cluster arrangement with balanced spacing and unified flow.'
    );
  }
  promptLines.push(
