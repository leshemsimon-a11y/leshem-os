...
// ---------------------------------------------------------------------------
// buildOutputPack(project, workAssets?) → { professionalHe, mediaPromptEn,
//   clientHe, assets, references }
// Clean 7B: optionally asset-aware — pass the project's normalized Work
// Assets (from lib/studio/assetContext) to enrich all sections. Without
// them the pack behaves exactly as in Clean 7A.
// ---------------------------------------------------------------------------
export function buildOutputPack(project, workAssets) {
  const p = project || {};
  const brief = p.brief || {};
  ...
  const assets = Array.isArray(workAssets) ? workAssets : [];
  const imageAssets = assets.filter((a) => ['image', 'sketch', 'render'].includes(a.type));
  const modelAssets = assets.filter((a) => a.type === 'model3d' && a.fileType !== '3dm');
  const rhinoAssets = assets.filter((a) => a.fileType === '3dm');
  const stoneCount = trayItems.length;
  ...
  // Clean 7B — asset-aware context in the professional summary.
  if (assets.length) {
    profLines.push('');
    if (imageAssets.length && (modelAssets.length || rhinoAssets.length)) {
      profLines.push('העיצוב מתבסס על רפרנסים חזותיים וקובצי מודל שהועלו לתיק.');
    } else if (imageAssets.length) {
      profLines.push('העיצוב מתבסס על רפרנסים חזותיים שהועלו לתיק.');
    } else {
      profLines.push('העיצוב מתבסס על קובצי מודל שהועלו לתיק.');
    }
  }
  // Clean 7B — cluster as a possible direction when multiple stones exist
  // (text-level only; no enum/schema change).
  if (stoneCount >= 2 && brief.stoneUsage !== 'none') {
    profLines.push('כיוון אפשרי נוסף: שיבוץ קלאסטר (Cluster) סביב מספר האבנים שבתיק.');
  }
  ...
  // Clean 7B — asset-aware prompt context.
  if (imageAssets.length) {
    promptLines.push('Use the uploaded jewelry reference image as visual inspiration.');
  }
  if (modelAssets.length) {
    promptLines.push('Use the uploaded model file as a base jewelry form/reference.');
  }
  if (rhinoAssets.length) {
    promptLines.push('A 3DM model is attached; preview support is planned.');
  }
  if (stoneCount >= 2 && brief.stoneUsage !== 'none') {
    promptLines.push('A cluster arrangement of the selected stones is a possible design direction.');
  }
  promptLines.push(
  ...
  // Clean 7B — mention uploaded references in the client-facing text.
  if (imageAssets.length || modelAssets.length || rhinoAssets.length) {
    clientHe += ' העיצוב נשען על רפרנסים וקבצים שהועברו אלינו, לדיוק מלא מול הציפייה.';
  }
  clientHe += ' העבודה מלווה באופן אישי משלב הרעיון ועד הביצוע.';

  // -------------------------------------------------------------------
  // Clean 7B — Work Assets section (name · type · role · preview status).
  // -------------------------------------------------------------------
  const previewStatusHe = (a) => {
    if (['image', 'sketch', 'render'].includes(a.type)) {
      return a.previewUrl ? 'תצוגה זמינה' : 'ללא תצוגה מקדימה';
    }
    if (a.fileType === '3dm') return 'תצוגת 3DM תהיה זמינה בהמשך';
    if (a.type === 'model3d') return 'תצוגה תלת־ממדית זמינה';
    return 'ללא תצוגה מקדימה';
  };
  const assetLines = assets.map(
    (a) => `• ${a.name} — ${a.fileType ? a.fileType.toUpperCase() : 'קובץ'} · ${a.role} · ${previewStatusHe(a)}`
  );

  // -------------------------------------------------------------------
  // References — real Work Assets when present, existing linkage counts
  // otherwise, else the honest placeholder (no schema change, no upload).
  // -------------------------------------------------------------------
  const linkedFilesCount = Array.isArray(p.linkedAssetFileIds) ? p.linkedAssetFileIds.length : 0;
  const references = {
    linkedFilesCount: assets.length || linkedFilesCount,
    title: HE.refsTitle,
    text: assets.length
      ? HE.refsLinkedFiles(assets.length)
      : linkedFilesCount > 0
        ? HE.refsLinkedFiles(linkedFilesCount)
        : HE.refsEmpty,
  };
  ...
  return {
    ...
    clientHe,
    assets: assetLines, // Clean 7B — formatted Work Asset lines (Hebrew)
    references,
  };
}
