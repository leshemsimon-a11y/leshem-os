...
// ===========================================================================
// Clean 5C — Structured, copy-ready render brief (additive)
// ===========================================================================
// A STRONGER render brief for the Stone-to-Jewelry Visual Workflow MVP. It
// produces (a) a flat object of named fields the UI shows with Hebrew labels,
// and (b) a single COPY-READY ENGLISH prompt suitable for AI visualization
// tools (English performs better there; this is an internal production artifact,
// NOT a customer certificate, so English is intended).
//
// Still PURE + LOCAL. No image generation, no external AI, no network — it only
// describes the shot in words. Real stone data (shape / color / carat / role
// placement) is used when present, with safe fallbacks and no fabrication.

const PRODUCT_TYPE_EN = {
  ring: 'ring',
  ...
  earrings: 'earrings',
  bracelet: 'bracelet',
  matchingPiece: 'matching/complementary piece',
  noStones: 'metal jewelry piece',
  other: 'fine jewelry piece',
};
...
const STYLE_EN = {
  ...
  geometric: 'geometric',
  // --- Clean 5E — additive style values ---
  luxury: 'luxury',
...
// Clean 5E — render/design freedom level, threaded into the ENGLISH render
// brief as one clear instruction sentence, and into the Hebrew brief line.
// Text only — no render engine is called; existing string fields only.
const FREEDOM_EN = {
  locked:
    'Design freedom: LOCKED — follow the provided stone data, reference and proportions as closely as possible; minimal creative deviation.',
  guided:
    'Design freedom: GUIDED — keep the core idea, stones, metal and product type; tasteful professional improvements are allowed.',
  creative:
    'Design freedom: CREATIVE — keep the selected stones and general product type; stronger design interpretation and alternative settings are welcome.',
  exploratory:
    'Design freedom: EXPLORATORY — use the selected stones as inspiration; bold, collection-style concepts are welcome while staying jewelry-realistic and production-aware.',
};

const FREEDOM_HE = {
  locked: 'רמת חופש: מדויק — צמוד לנתונים ולרפרנס, מינימום סטייה.',
  guided: 'רמת חופש: מאוזן — שמירה על הרעיון עם שיפורים מקצועיים.',
  creative: 'רמת חופש: פתוח — פרשנות עיצובית חזקה מותרת.',
  exploratory: 'רמת חופש: חופשי — כיוונים נועזים ברמת קולקציה, מודעים לייצור.',
};

function freedomKey(brief) {
  return isValidFreedomLevel(brief.freedomLevel)
    ? brief.freedomLevel
    : DEFAULT_FREEDOM_LEVEL;
}

// Hebrew gem-shape → English (best effort; falls through to the raw value).
const SHAPE_EN = {
...
function enProductType(pt) {
  return PRODUCT_TYPE_EN[pt] || 'fine jewelry piece';
}
function enMetal(brief) {
  const m = brief.metalPreference;
  return (m && METAL_EN[m]) || 'fine jewelry metal (white/yellow gold or platinum)';
}
function enStyle(brief) {
  const s = brief.styleDirection;
  return (s && STYLE_EN[s]) || 'refined';
}
function enShape(shapeHe) {
  if (!shapeHe || typeof shapeHe !== 'string') return '';
  return SHAPE_EN[shapeHe.trim()] || shapeHe.trim();
}

// Describe the stones for the render shot from real tray data when available.
function describeStonesEn(info, metalOnly) {
  if (metalOnly || !info.hasAnyStone) return '';
  const bits = [];
  const centers = Array.isArray(info.centers) ? info.centers : [];
  centers.forEach((it) => {
    const s = (it && it.snapshot) || {};
    const shape = enShape(s.shapeHe);
    const color = typeof s.color === 'string' ? s.color : '';
    const carat = typeof s.caratWeight === 'number' ? `${s.caratWeight}ct` : '';
    const desc = [carat, color, shape, 'center stone'].filter(Boolean).join(' ');
    if (desc.trim()) bits.push(desc.trim());
  });
  if (!centers.length && info.hasAnyStone) {
    bits.push('selected stones arranged as the focal composition');
  }
  if (info.hasSide) bits.push('accent side stones flanking the center');
  if (info.hasAccent) bits.push('additional accent stones');
  if (info.hasParcel) {
    bits.push('a few representative stones from the parcel (not the full parcel)');
  }
  return bits.join('; ');
}

// Clean 5C — structured render brief object + copy-ready English prompt.
function buildRenderBriefStructured(concept, brief, info, metalOnly, ptHe) {
  const jewelryType = enProductType(brief.productType || concept.productType);
  const metal = enMetal(brief);
  const style = enStyle(brief);
  const stonePlacement = describeStonesEn(info, metalOnly);

  const settingStyle = metalOnly || !info.hasAnyStone
    ? 'clean polished metalwork, no stone setting'
    : info.hasCenter
    ? 'precise prong/bezel setting holding the center stone, secure and elegant'
    : 'balanced setting integrating the selected stones';

  const proportions = metalOnly
    ? 'refined, true-to-life proportions with clean lines'
    : 'true-to-life jewelry proportions, stones in realistic scale to the metal';

  const cameraAngle = 'three-quarter hero angle with a complementary macro close-up';
  const lighting = info.hasAnyStone && !metalOnly
    ? 'soft studio lighting with controlled highlights to bring out brilliance and fire'
    : 'soft studio lighting with gentle reflections on the metal surface';
  const background = 'neutral seamless background (soft ivory or graphite), shallow depth of field';
  const styleDescriptor = `${style}, hyper-realistic luxury jewelry catalog photography, high detail, photorealistic`;

  const structured = {
    jewelryType,
    stonePlacement, // '' for metal-only
    metal,
    settingStyle,
    proportions,
    cameraAngle,
  ...
  // Copy-ready English prompt — single block, no duplicated chatter.
  const promptLines = [
    `A ${style} ${jewelryType} in ${metal}.`,
    stonePlacement ? `Stones: ${stonePlacement}.` : 'Focus on the metalwork and form.',
    ...
    // Clean 5E — the chosen freedom level as ONE clear instruction.
    FREEDOM_EN[freedomKey(brief)],
  ];
  const renderBriefText = promptLines.filter(Boolean).join(' ');

  return { structured, renderBriefText };
}
...
function buildRenderBrief(concept, brief, info, metalOnly, ptHe) {
  ...
  // Clean 5E — the chosen freedom level, one short Hebrew line.
  parts.push(FREEDOM_HE[freedomKey(brief)]);
...
export function generateOutput(trayItems, brief) {
  ...
  const rb = buildRenderBriefStructured(concept, b, info, metalOnly, ptHe);
  const raw = {
    ...
    // Clean 5C — structured + copy-ready English render brief (additive).
    renderBriefStructured: rb.structured,
    renderBriefText: rb.renderBriefText,
