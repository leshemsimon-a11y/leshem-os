...
function pickPrimaryImage(files) {
  const list = Array.isArray(files) ? files : [];
  const approvedImg = list.find((f) => f.fileKind === 'image' && f.status === 'approved');
  if (approvedImg) return approvedImg;
  const anyImg = list.find((f) => f.fileKind === 'image' && f.status !== 'archived');
  return anyImg || null;
}
...
export function assetObjectToTrayItem(object, files) {
  ...
  const primary = pickPrimaryImage(files);
