...
// ---------------------------------------------------------------------------
// buildRenderPackage(project, preset, sceneOverride) → structured render
// package. Pure: reads the project + shared prompt context; nothing is
// stored here.
//
// Clean 8J — sceneOverride (OPTIONAL, additive, fully backward-compatible):
// when lib/studio/renderSceneLibrary.js needs to adjust the opener/closing
// sentence and recommended settings for a specific scene/pack/quality
// combination, it passes a plain object here instead of duplicating all of
// this function's subject/stone/cluster/direction/asset/warning logic.
// Every existing call site — buildRenderPackage(project, preset) — is
// UNCHANGED: sceneOverride defaults to null, so every field below falls back
// to the exact preset-profile behavior this function already had.
//   sceneOverride?: {
//     openerEn?: string,        // replaces profile.openerEn
//     sceneEn?: string,         // replaces profile.sceneEn (closing line)
//     aspectRatio?: string,     // replaces the preset's recommendedAspectRatio
//     outputCount?: number,     // replaces the preset's recommendedOutputCount
//     quality?: string,         // replaces the preset's recommendedQuality
//     suggestedTool?: string,   // replaces the preset's suggestedTool
//   }
// ---------------------------------------------------------------------------
export function buildRenderPackage(project, preset, sceneOverride) {
  const p = project || {};
  const brief = p.brief || {};
  ...
  const ov = sceneOverride && typeof sceneOverride === 'object' ? sceneOverride : null;
  const openerEn = ov && typeof ov.openerEn === 'string' && ov.openerEn ? ov.openerEn : profile.openerEn;
  const sceneEn = ov && typeof ov.sceneEn === 'string' && ov.sceneEn ? ov.sceneEn : profile.sceneEn;
  ...
  lines.push(`${openerEn}: a ${subject} crafted in ${metalEn}.`);
  ...
  // Clean 8J — scene override for recommended settings (falls back to the
  // existing preset-driven settings when no override field is supplied, so
  // pre-8J callers see byte-identical output).
  const recommendedAspectRatio =
    ov && typeof ov.aspectRatio === 'string' && ov.aspectRatio
      ? ov.aspectRatio
      : settings.recommendedAspectRatio;
  const recommendedOutputCount =
    ov && typeof ov.outputCount === 'number' && ov.outputCount > 0
      ? ov.outputCount
      : settings.recommendedOutputCount;
  const recommendedQuality =
    ov && typeof ov.quality === 'string' && ov.quality ? ov.quality : settings.recommendedQuality;
  const suggestedTool =
    ov && typeof ov.suggestedTool === 'string' && ov.suggestedTool
      ? ov.suggestedTool
      : settings.suggestedTool;
