...
// Add a PRE-BUILT tray item (e.g. produced by the asset workflow bridge).
// Backward-compatible companion to addToTray (which builds items from raw
// inventory assets). De-dupes by the item's own id. Inventory behavior is
// unchanged. The item must already match the tray item shape.
export function addItemToTray(item) {
  if (!item || typeof item !== 'object' || typeof item.id !== 'string' || !item.id) {
    console.warn('[workTray] addItemToTray needs an item with a stable id.');
    return getTray();
  }
  const list = loadInitial();
  if (list.some((it) => it.id === item.id)) {
    return [...list]; // already present — distinct items only
  }
  if (list.length >= MAX_ITEMS) {
    console.warn('[workTray] tray is at capacity; not adding more.');
    return [...list];
  }
  const safe = { role: 'unassigned', addedAt: Date.now(), ...item };
  const next = [...list, safe];
  persist(next);
  return next;
}
...
// Replace the entire tray with a known-good list (used when OPENING a saved
// Design Project back into the studio). Additive API; existing add/remove/
// setRole/clear behavior is unchanged. Items are filtered to well-formed
// entries with a stable id, exactly like the loader does.
export function replaceTray(items) {
...
export function createUseWorkTray(React) {
  ...
  return function useWorkTray() {
    ...
    const api = {
      ...
      replace: useCallback((items) => setItems(replaceTray(items)), []),
      addItem: useCallback((item) => setItems(addItemToTray(item)), []),
