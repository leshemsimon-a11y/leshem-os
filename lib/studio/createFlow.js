...
// Clean 8K-R4 — additive `export`: the golden path's product-type
// enforcement (lib/studio/goldenPath.js) needs the SAME mapping used here to
// independently compute the expected brief-level productType BEFORE
// generation, then check the generator's output against it. No behavior
// change — this constant already existed; it is simply now also visible.
...
// Clean 8K-R4 — canonical metal display maps (English values match the
// EXISTING METAL_PREFERENCE enum in designDraft.js verbatim; no new values).
const METAL_HE = Object.freeze({
  whiteGold: 'זהב לבן',
  yellowGold: 'זהב צהוב',
  roseGold: 'זהב אדום',
  platinum: 'פלטינה',
...
export function generateCreateDirections(input) {
  const { product, style, trayItems, referenceText, requestText, metalPreference } = input || {};
  ...
  // Clean 8K-R4 — metal preference, if known (existing METAL_PREFERENCE
  // values only). Purely additive: absent metal degrades to the previous
  // "in precious metal" phrasing exactly as before.
  const metalHe = METAL_HE[metalPreference] || '';
  const metalEn = METAL_EN[metalPreference] || null;

  const enBase = [
    `A ${[sEn, pEn].filter(Boolean).join(' ')} in ${metalEn || 'precious metal'}`,
...
export function buildCreateBrief(input, directions, selectedDirectionId) {
  ...
  return {
    ...
    // Clean 8K-R4 — existing METAL_PREFERENCE value or null; additive field,
    // already-valid on the existing brief schema (designDraft.js), so this
    // is not a new persistence key or schema change.
    metalPreference: METAL_HE[metalPreference] ? metalPreference : null,
...
export function buildCreateOutputPack(input, directions, selectedDirectionId) {
  ...
  // Clean 8K-R4 — metal preference, if known (additive; absent metal
  // degrades to the previous copy exactly as before).
  const metalHe = METAL_HE[metalPreference] || null;
  const metalEn = METAL_EN[metalPreference] || null;
  ...
  const profLines = [
    ...
    '',
    `תכשיט: ${pHe}${sHe ? ` · סגנון: ${sHe}` : ''}${metalHe ? ` · מתכת: ${metalHe}` : ''}`,
    '',
    'אבנים ופריטים:',
  ...
  const stonesEn = stonesEnList(items);
  const promptLines = [
    `Professional jewelry visualization: a ${[sEn, pEn].filter(Boolean).join(' ')} in ${metalEn || 'precious metal'}.`,
  ];
  ...
  let clientHe = `${pHe} בעיצוב אישי${sHe ? ` בקו ${sHe}` : ''}${metalHe ? ` ב${metalHe}` : ''}.`;
